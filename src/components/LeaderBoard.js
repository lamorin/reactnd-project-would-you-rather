  ...
  return (
      <Container maxWidth="md" align="center">
        <Paper style={{padding: '2rem'}}>
          { _.reverse(_.sortBy(_.values(users), (user) => (user.createdQuestions + user.answeredQuestions))).map((user)=>(<LeaderBoardMember key={user.id} user={user} createdQuestions={user.createdQuestions} answeredQuestions={user.answeredQuestions}></LeaderBoardMember>)) }
        </Paper>
      </Container>
  );
}
